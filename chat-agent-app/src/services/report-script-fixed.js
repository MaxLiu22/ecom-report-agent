...
class DiReportGenerator {
    constructor() {
        this.uploadedData = {};
        this.processedData = {};
        // 汇总最终可用于展示的结果（含表格数据和概述 HTML）
        this.reportData = {};
        this.init();
    }

    init() { /* no-op: 移除页面级 DOM 事件绑定 */ }

    normalizeFileName(fileName) {
        return fileName.toLowerCase().replace(/[^a-z0-9]/g, '');
    }

    async handleFileUpload(eventOrFiles) {
        // 兼容多种输入：1) File[] 2) FileList 3) { files: FileList } 4) 事件对象(event.target.files)
        if (!eventOrFiles) {
            console.warn('handleFileUpload called with empty input');
            return;
        }
        let files = [];
        if (Array.isArray(eventOrFiles)) {
            files = eventOrFiles;
        } else if (eventOrFiles && typeof eventOrFiles.length === 'number' && typeof eventOrFiles.item === 'function') {
            // 直接传入 FileList
            files = Array.from(eventOrFiles);
        } else if (eventOrFiles && eventOrFiles.files && typeof eventOrFiles.files.length === 'number') {
            // 传入形如 { files: FileList } 或 input 元素
            files = Array.from(eventOrFiles.files);
        } else if (eventOrFiles && eventOrFiles.target && eventOrFiles.target.files && typeof eventOrFiles.target.files.length === 'number') {
            // 事件对象
            files = Array.from(eventOrFiles.target.files);
        } else {
            console.warn('handleFileUpload called with unsupported input:', eventOrFiles);
            return;
        }
    ...
    checkRequiredFiles() {
        ...
        // 不再操作 DOM；返回检测结果供上层使用
        return hasAllRequired;
    }

    generateReport() {
        // 保留兼容入口：仅执行数据处理并返回结构化结果
        this.processData();
    ...
    processData() {
        ...
        // 在计算完成后立即生成结构化报告数据，便于上层直接读取 this.reportData
        try {
            this.reportData = this.renderReport();
        } catch (e) {
            console.warn('renderReport failed after processData, reportData remains empty:', e);
        }
    }
    ...
    renderReport() {
        // 仅构建结构化数据（无 DOM 注入）
        const opportunitySummaryHtml = this.renderOpportunitySummary();
        const ukToEuTable = this.renderUkToEuTable();
        const euToUkTable = this.renderEuToUkTable();

        // 汇总结构化输出，便于外部直接用于展示
        this.reportData = {
            opportunitySummaryHtml,
            ukToEuTable,
            euToUkTable,
            // 可选透传：保留已计算的原始聚合数据，便于上层做更多展示
    ...
    renderOpportunitySummary() {
        const ukToEu = this.processedData.ukToEu;
        const euToUk = this.processedData.euToUk;
        
        let summaryHtml = '<div class="summary-content">';
        
        summaryHtml += `<h4>UK > EU 机会点</h4>`;
        summaryHtml += `<p><strong>同步入库欧盟增长机会：</strong>卖家有${ukToEu.ukOnlyLocal.count}个选品仅在英国本地入库销售，未在欧盟入库。其中${ukToEu.highPotentialEu.count}个选品被选品指南针推荐同步入库至欧盟，预计可在欧盟销售${ukToEu.highPotentialEu.potentialSales}(90天)。详情可下载选品指南针报告查看推荐清单。</p>`;
        
        if (ukToEu.gsiCredits && ukToEu.gsiCredits.totalCredits > 0) {
            summaryHtml += `<p><strong>全球拓展大礼包激励：</strong>若将指定英国头部选品同步入库至欧盟，即可获取最高€${ukToEu.gsiCredits.totalCredits}代金券用于抵扣欧盟的配送费和佣金。</p>`;
        }
        
        summaryHtml += `<p><strong>开启远程配送测试销售：</strong>开启英国至欧盟的远程配送，测试选品在欧盟的表现。</p>`;
        
        summaryHtml += `<h4>EU > UK 机会点</h4>`;
        summaryHtml += `<p><strong>同步入库英国增长机会：</strong>卖家有${euToUk.euOnlyLocal.count}个选品仅在欧盟本地入库销售，未在英国入库。其中${euToUk.highPotentialUk.count}个选品被选品指南针推荐同步入库至英国，预计可在英国销售${euToUk.highPotentialUk.potentialSales}(90天)。详情可下载选品指南针报告查看推荐清单。</p>`;
        
        if (euToUk.gsiCredits && euToUk.gsiCredits.totalCredits > 0) {
            summaryHtml += `<p><strong>全球拓展大礼包激励：</strong>若将指定欧盟头部选品同步入库至英国，即可获取最高£${euToUk.gsiCredits.totalCredits}代金券用于抵扣英国的配送费和佣金。</p>`;
        }
        
        summaryHtml += `<p><strong>开启远程配送测试销售：</strong>开启欧盟至英国的远程配送，测试选品在英国的表现。</p>`;
    ...
    renderUkToEuTable() {
        const data = this.processedData.ukToEu;
        
        // 构造结构化行数据
        const rows = [];
        rows.push({
            metric: '仅在英国本地入库的选品',
            count: data.ukOnlyLocal.count,
            revenue: data.ukOnlyLocal.revenue,
            action: '-'
        });
        rows.push({
            metric: '选品指南针推荐同步入库至欧盟',
            count: data.highPotentialEu.count,
            revenue: data.highPotentialEu.revenue,
            action: `本地入库至欧盟，预计销售额提升${data.highPotentialEu.potentialSales}（90天内）`
        });
        if (data.gsiCredits && data.gsiCredits.totalCredits > 0) {
            rows.push({
                metric: '在欧盟有全球拓展大礼包',
                count: data.gsiCredits.cAsinCount,
                revenue: data.gsiCredits.revenue || '£0',
                action: `同步${data.gsiCredits.pAsinCount}个P-ASIN，对应${data.gsiCredits.cAsinCount}个C-ASIN，可领取€${data.gsiCredits.totalCredits}代金券`
            });
        }
        rows.push({
            metric: '未开启英国到欧盟的远程配送',
            count: data.notRemote.count,
            revenue: data.notRemote.revenue,
            action: '将ASIN同步至欧盟，并开启英国至欧盟的远程配送'
        });
        const remoteRevenueDisplay = data.remoteActive.remoteRevenue >= 1000
            ? `已在欧盟产生销售额€${Math.round(data.remoteActive.remoteRevenue).toLocaleString()}，建议入库至欧盟，享受本地配送费`
            : '-';
        rows.push({
            metric: '已开启英国到欧盟的远程配送',
            count: data.remoteActive.count,
            revenue: data.remoteActive.revenue,
            action: remoteRevenueDisplay
        });

        // 返回结构化表格数据
        return {
            headers: ['类别', 'ASIN数量', 'T30D销售额', '建议动作'],
            rows
        };
    }

    renderEuToUkTable() {
        const data = this.processedData.euToUk;
        
        // 构造结构化行数据
        const rows = [];
        rows.push({
            metric: '仅在欧盟本地入库的选品',
            count: data.euOnlyLocal.count,
            revenue: data.euOnlyLocal.revenue,
            action: '-'
        });
        rows.push({
            metric: '选品指南针推荐同步入库至英国',
            count: data.highPotentialUk.count,
            revenue: data.highPotentialUk.revenue,
            action: `本地入库至英国，预计销售额提升${data.highPotentialUk.potentialSales}（90天内）`
        });
        if (data.gsiCredits && data.gsiCredits.totalCredits > 0) {
            rows.push({
                metric: '在英国有全球拓展大礼包',
        ...
        rows.push({
            metric: '未开启欧盟至英国的远程配送',
            count: data.notRemote.count,
            revenue: data.notRemote.revenue,
            action: '将ASIN同步至英国，并开启欧盟至英国的远程配送'
        });
        const remoteRevenueDisplay = data.remoteActive.remoteRevenue >= 1000
            ? `已在英国产生销售额£${Math.round(data.remoteActive.remoteRevenue).toLocaleString()}，建议入库至英国，享受本地配送费`
            : '-';
        rows.push({
            metric: '已开启欧盟至英国的远程配送',
            count: data.remoteActive.count,
            revenue: data.remoteActive.revenue,
            action: remoteRevenueDisplay
        });
